...
public class CreateDerbyDB
{
	...
	private void createTable(Connection conn) throws SQLException
	{
		...
		executeCommand(conn, "CREATE TABLE accessibletbl(textcol VARCHAR(6))");
		executeCommand(conn, "INSERT INTO accessibletbl VALUES('hello')");
		println("Granting select/insert privileges to adder...");
		executeCommand(conn, "GRANT SELECT, INSERT ON accessibletbl TO adder");
		println("Granting select privileges to viewer");
		executeCommand(conn, "GRANT SELECT ON accessibletbl TO viewer");
	}

...
public class WarehouseAction
{
	public void pagingAction(int currentPage)
	{
		...
		new SwingWorker<List<String[]>, Void>()
		{
			...
			protected List<String[]> doInBackground() throws Exception// 查找当前的所有列表
			{
				...
				List<Map<String, Object>> maplist = WarehouseService.getInstance().getPaging(currentPage);
				for (Map<String, Object> map : maplist)
				{
					List<String> strTempList = new ArrayList<>();
					for (String string : ConstantsDB.WareHouseHeadDB)
					{
	...
	public void refreshAction()
	{
		...
		try
		{
			count = DaoUtil.getInstance().countTableRows("warehouselist");
	...
	public void deleteAction()
	{
		...
		new SwingWorker<Boolean, Void>()
		{
			...
			{
				return WarehouseService.getInstance().deleteRows(IDs);
			}
			@Override
			protected void done()
			{
	...
	public void modifyAction(Map<String, String> map)
	{
		...
		List<String> list = Arrays.asList(ConstantsDB.WareHouseHead);
		for(Map.Entry<String, String> entry : map.entrySet())
		{
			int listIndex = list.indexOf(entry.getKey());
			System.out.println("key :"+ConstantsDB.WareHouseHeadDB[listIndex]+" value : "+entry.getValue());
			newMap.put(ConstantsDB.WareHouseHeadDB[listIndex], entry.getValue());
		}
		new SwingWorker<Boolean, Void>()
		{
			@Override
			protected Boolean doInBackground() throws Exception//查找当前的所有列表
			{
				return WarehouseService.getInstance().modifyRows(IDs, newMap);
			}
			@Override
			protected void done()
			{

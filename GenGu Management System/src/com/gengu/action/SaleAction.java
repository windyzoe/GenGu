...
public class SaleAction
{
	public void pagingAction(int currentPage)
	{
		...
		new SwingWorker<List<String[]>, Void>()
		{
			...
			protected List<String[]> doInBackground() throws Exception// 查找当前的所有列表
			{
				...
				List<Map<String, Object>> maplist = SaleService.getInstance().getPaging(currentPage);
				for (Map<String, Object> map : maplist)
				{
					List<String> strTempList = new ArrayList<>();
					for (String string : ConstantsDB.SaleHeadDB)
					{
			...
			protected void done()
			{
				DefaultTableModel tableModel = (DefaultTableModel) MainFrame.getInstance().saleTable.getModel();
				...
				try
				{
					...
					JTableUtil.fitTableColumns(MainFrame.getInstance().saleTable);// 自适应宽度
					// 刷新ToolTip
	...
	public void refreshAction()
	{
		...
		try
		{
			count = DaoUtil.getInstance().countTableRows("salelist");
	...
	public void deleteAction()
	{
		...
		new SwingWorker<Boolean, Void>()
		{
			...
			{
				return SaleService.getInstance().deleteRows(IDs);
			}
			@Override
			protected void done()
			{
	...
	public void modifyAction(Map<String, String> map)
	{
		...
		List<String> list = Arrays.asList(ConstantsDB.SaleHead);
		for(Map.Entry<String, String> entry : map.entrySet())
		{
			int listIndex = list.indexOf(entry.getKey());
			System.out.println("key :"+ConstantsDB.SaleHeadDB[listIndex]+" value : "+entry.getValue());
			newMap.put(ConstantsDB.SaleHeadDB[listIndex], entry.getValue());
		}
		new SwingWorker<Boolean, Void>()
		{
			@Override
			protected Boolean doInBackground() throws Exception//查找当前的所有列表
			{
				return SaleService.getInstance().modifyRows(IDs, newMap);
			}
			@Override
			protected void done()
			{
